...
public class GetFreeNodePortsKlient {
	public static void main(String[] args) {
		...
		/*
		String uri="http://localhost:8080/KubernetesFabric8RESTapi/API/v01/getFreeNodePorts";
		//String uri="http://194.249.0.47:8080/KubernetesFabric8RESTapi/API/v01/getFreeNodePorts";
		//String uri="http://localhost:9999/KubernetesFabric8RESTapi/API/v01/getFreeNodePorts"; //used for TCP/IP monitor in Eclipse - the monitor should be configured properly to forward packets to final server
		...
		if(response.getStatus()!=200){
			...
			String theSerializedListOfPorts = response.readEntity(String.class);
			System.out.println("Torej cluster z IDjem "+clusterId+" ima proste NodePort porte: "+ theSerializedListOfPorts);
		}
		 */

		//%%%%%%%%%%%%%%%%%%%%%%%%%OR RATHER USE OPTION 2 WHICH IS MORE RESTFULL%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		String namespace="default";
		String uri="http://localhost:8080/KubernetesFabric8RESTapi/API/v01/"+clusterId+"/namespaces/"+namespace+"/availableNodePorts";
		//ClientConfig config = new ClientConfig().register(JacksonFeatures.class);
		ClientConfig config = new ClientConfig().register(LoggingFilter.class);
		Client client = ClientBuilder.newClient(config);
		WebTarget service = client.target(UriBuilder.fromUri(uri).build());
		...
		if(response.getStatus()!=200){
			...
			String theSerializedListOfAvailableNodePorts = response.readEntity(String.class);
			System.out.println("V clusterju "+clusterId+" in namespejsu "+namespace+" so na voljo sledeci prosti NodePort porti: "+ theSerializedListOfAvailableNodePorts);
		}
	}
