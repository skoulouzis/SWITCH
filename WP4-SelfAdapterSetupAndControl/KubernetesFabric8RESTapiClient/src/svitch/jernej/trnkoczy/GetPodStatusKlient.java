...
public class GetPodStatusKlient {
	public static void main(String[] args) {
		...
		/*
		String uri="http://localhost:8080/KubernetesFabric8RESTapi/API/v01/getPodStatusInDefaultNamespace";
		//String uri="http://194.249.0.47:8080/KubernetesFabric8RESTapi/API/v01/getPodStatusInDefaultNamespace";
		//String uri="http://localhost:9999/KubernetesFabric8RESTapi/API/v01/getPodStatusInDefaultNamespace"; //used for TCP/IP monitor in Eclipse - the monitor should be configured properly to forward packets to final server
		...
		//---------------------SPECIFICATION OF THE POD TO FIND IT's IP - IT CONTAINS THE CLUSTER ID AND THE NAME OF THE POD --------------
		...
		if(response.getStatus()!=200){
			...
			String thePodStatus = response.readEntity(String.class);
			System.out.println("Pod z imenom "+podName+" ima trenutno status: "+ thePodStatus);
		}
		 */	
		...
		String uri="http://localhost:8080/KubernetesFabric8RESTapi/API/v01/"+clusterId+"/namespaces/"+namespace+"/pods/"+podName+"/status";
		//ClientConfig config = new ClientConfig().register(JacksonFeatures.class);
		ClientConfig config = new ClientConfig().register(LoggingFilter.class);
		Client client = ClientBuilder.newClient(config);
		WebTarget service = client.target(UriBuilder.fromUri(uri).build());
		...
		if(response.getStatus()!=200){
			...
			String podStatus = response.readEntity(String.class);
			System.out.println("Pod z imenom "+podName+" ima trenutni status: "+ podStatus);
		}
