...
public class StopPodKlient {
	...
	public static void main(String[] args) {
		...
		/*
		String uri="http://localhost:8080/KubernetesFabric8RESTapi/API/v01/stopPodByNameInDefaultNamespace";
		//String uri="http://194.249.0.47:8080/KubernetesFabric8RESTapi/API/v01/stopPodByNameInDefaultNamespace";
		//String uri="http://localhost:9999/KubernetesFabric8RESTapi/API/v01/stopPodByNameInDefaultNamespace"; //used for TCP/IP monitor in Eclipse - the monitor should be configured properly to forward packets to final server
		...
		//---------------------SPECIFICATION OF THE POD TO DELETE - IT CONTAINS THE CLUSTER ID AND THE NAME OF THE POD TO DELETE IN THIS CLUSTER--------------
		...
		if(response.getStatus()!=200){
			...
			System.out.println("Ravnokar smo poslali ukaz za izbris pod-a: "+ theNameOfDeletedPod);
		}
		 */
		...
		String podName="imetegalepoda";
		String uri="http://localhost:8080/KubernetesFabric8RESTapi/API/v01/"+clusterId+"/namespaces/"+namespace+"/pods/"+podName;
		//ClientConfig config = new ClientConfig().register(JacksonFeatures.class);
		ClientConfig config = new ClientConfig().register(LoggingFilter.class);
		Client client = ClientBuilder.newClient(config);
		WebTarget service = client.target(UriBuilder.fromUri(uri).build());
		...
		if(response.getStatus()!=200){
			...
			String theNameOfDeletedPod = response.readEntity(String.class);
			System.out.println("Pod katerega izbris smo ravnokar zahtevali ima ime: "+theNameOfDeletedPod);
		}
