...
public class GetPodHostIPKlient {
	public static void main(String[] args) {
		...
		/*
		String uri="http://localhost:8080/KubernetesFabric8RESTapi/API/v01/getPodHostIPInDefaultNamespace";
		//String uri="http://194.249.0.47:8080/KubernetesFabric8RESTapi/API/v01/getPodHostIPInDefaultNamespace";
		//String uri="http://localhost:9999/KubernetesFabric8RESTapi/API/v01/getPodHostIPInDefaultNamespace"; //used for TCP/IP monitor in Eclipse - the monitor should be configured properly to forward packets to final server
		...
		if(response.getStatus()!=200){
			...
			String theIPofPod = response.readEntity(String.class);
			System.out.println("Host IP pod-a z imenom "+podName+" je: "+ theIPofPod);
		}
		 */
		...
		String uri="http://localhost:8080/KubernetesFabric8RESTapi/API/v01/"+clusterId+"/namespaces/"+namespace+"/pods/"+podName+"/hostIP";
		//ClientConfig config = new ClientConfig().register(JacksonFeatures.class);
		ClientConfig config = new ClientConfig().register(LoggingFilter.class);
		Client client = ClientBuilder.newClient(config);
		WebTarget service = client.target(UriBuilder.fromUri(uri).build());
		...
		if(response.getStatus()!=200){
			...
			String theIPofPod = response.readEntity(String.class);
			System.out.println("Pod z imenom "+podName+" v namespejsu "+namespace+" in clusterju "+clusterId+" tece na hostu z external IP-jem: "+theIPofPod);
		}
